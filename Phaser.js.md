...
`config` 속성
1. `classType`: 그룹에 추가되는 객체의 기본 클래스를 지정합니다. 지정하지 않으면 기본적으로 `Phaser.GameObjects.GameObject`가 사용됩니다.
...
객체 풀링을 사용하는 그룹에서 비활성화된 객체가 없을 때 `get()` 메서드는 새로운 객체를 생성하여 반환합니다. 객체가 풀에 없으면, 그룹의 `classType`에 설정된 기본 타입으로 새로운 객체가 만들어집니다. (객체 풀링이 비활성화된 경우, `get()` 메서드는 `null`을 반환합니다.)
