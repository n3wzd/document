# HTML
`HTML`은 HyperText Markup Language의 약자로, 웹페이지를 기술하는 마크업 언어다.

## 요소 (element)
HTML 문서는 `요소(element)`들로 구성되어 있으며, 요소의 구조는 다음과 같다.
...
- `태그(tag)`는 해당 요소의 종류와 범위를 나타낸다.
	- 태그는 `시작 태그(start tag)`와 `종료 태그(end tag)`로 구성된다.
	- 일부 요소는 시작 태그만 존재하기도 한다. (ex. img, br 등)
- `속성(attribute)`은 해당 요소에 추가적인 정보를 제공한다.
	- 속성은 시작 태그 내에서 작성되며, 한 요소가 여러 개의 속성을 가질 수 있다.
	- 속성은 이름과 값으로 구성된다. `<name="value">`
		- 이름은 소문자로 작성하는 것이 권장된다.
		- 값은 따옴표를 사용해서 감싼다.

## 텍스트 (text)
### 제목 (Heading)
#### h
`<h>`는 제목을 표현한다. (heading)
...
### 단락 (Paragraph)
#### p
`<p>`는 단락을 표현한다. (paragraph)

- 여러 개의 공백과 줄바꿈은 1개의 공백으로 표현된다.
...
#### pre
`<pre>`는 코드에 작성된 텍스트 서식이 반영된 단락을 표현한다. (preformatted)

- 내용에 있는 공백과 줄바꿈을 그대로 반영한다.
- 글꼴은 고정폭으로 자동변환된다.
...
#### hr
`<hr>`은 수평 가로 구분선이다. (horizontal rule)
...
### 서식 (Formatting)
#### b, strong
`<b>`, `<strong>`은 볼드체로 표현해준다.

- strong은 내용의 중요성을 추가로 포함하고 있다. (=검색엔진 크롤러가 우선시함)
...
#### i, em
`<i>`, `<em>`은 기울임체(이탤릭체)로 표현해준다.

- em은 내용의 중요성을 추가로 포함하고 있다. (emphasized)
...
#### sup, sub
`<sup>`, `<sub>`는 각각 위첨자, 아래첨자를 표현한다. (superscript, subscript)
...
#### blockquote
`<blockquote>`는 블록 인용구(긴 인용문)르 표현한다.
...
#### abbr
`<abbr>`는 축약형을 표현한다. (arreviation)

|속성|값|
|---|---|
|title|내용에 마우스를 올리면 해당 값이 표시된다|
...
### 엔티티 (Entity)
`HTML 예약어(reserved characters)`는 HTML에서 미리 예약된 문자로, 특별한 용도로 해석된다. `엔티티(entity)`는 HTML 예약어를 단순 문자로 사용하게 해준다.
...
## 스타일 (Style)
### 스타일 (Style)
`style` 속성을 사용하면 CSS 스크립트를 요소에 적용할 수 있다.
...
### 배경 (Background)
HTML에서 배경을 설정하는 방법은 2가지가 있다.

1. 배경을 색으로 설정
2. 배경을 이미지로 설정

#### 색 설정
CSS를 사용한다. (ex. CSS의 `background-color` 속성)
...
- img의 스타일 관련 설정(크기, 테두리 등)은 CSS로 하는 것이 좋다.

### 리스트 (List)
HTML 리스트는 3가지 종류가 있다.
...
#### ul
`<ul>`을 사용해서 순서가 없는 리스트를 생성할 수 있다. `<li>`는 리스트 요소를 담당한다.
...
#### ol
`<ol>`을 사용해서 순서가 있는 리스트를 생성할 수 있다. `<li>`는 리스트 요소를 담당한다.
...
#### dl
`<dl>`을 사용해서 정의 리스트를 생성할 수 있다. `<dt>`는 용어의 이름, `<dd>`는 용어의 설명을 담당한다.
...
|속성|값|
|---|---|
|colspan|현재 열부터 n만큼 떨어진 열을 병합한다|
|rowspan|현재 행부터 n만큼 떨어진 행을 병합한다|
...
### iframe
`<iframe>`을 사용하면 HTML 문서 내부에 또다른 웹 페이지를 삽입할 수 있다. (inline frame)
...
`<a>`를 사용해서 iframe가 표시하는 페이지를 바꿀 수 있다.

## 레이아웃 (Layout)
레이아웃(layout)은 특정 공간에 여러 구서 요소를 효과적으로 배치하는 작업이다. HTML에서 보편적인 레이아웃 작성법은 2가지가 있다.

1. div 사용
2. HTML5 레이아웃

### div 레이아웃
`<div>`를 사용해서 레이아웃을 커스텀한다. 구성 요소별로 CSS 스타일을 적용할 수 있다는 이점이 있다.
...
### HTML5 레이아웃
HTML5에선 레이아웃을 위한 별도의 요소인 의미(semantic) 요소를 제공한다. (자세한 내용은 HTML5 - 의미 (Semantic)을 참고)

## 입력 (Input)
### form
`<form>`은 사용자의 입력 데이터를 서버 측의 페이지, 즉 폼 핸들러(form handler)로 전송할 수 있다.
...
- **GET**: 주소에 데이터를 포함하여 전달
	- 데이터가 주소를 통해 외부에 드러남
	- 전달 가능한 데이터의 크기가 제한됨
	- 크기가 작고 중요도가 낮은 데이터에 적합
- **POST**: 데이터를 별도로 첨부하여 전달
	- 데이터가 외부로부터 감춰짐
	- 전달 가능한 데이터 크기에 제한이 없음
	- 보안성 및 활용서이 좋음
	- GET 이외의 데이터는 POST를 사용

### input
`<input>`은 사용자의 입력을 받을 수 있다.
...
#### 속성
`<input>`의 `type` 속성이나 별도의 요소로 여러 입력을 추가할 수 있다.
...
##### file
`accept` 속성을 통해 업로드할 파일의 종류를 제한할 수 있다. 여러 개의 값을 추가한다면 or 연산자 `|`를 통해 묶을 수 있다.
...
##### select
`<option>` 요소로 드롭다운 리스트의 옵션을 넣을 수 있다.
...
##### fieldset
`<legend>` 요소로 fieldset의 제목을 표시할 수 있다.
...
적용 우선순위는 다음 순서와 같다.
1. 인라인 스타일
2. 내부/외부 스타일 시트: 둘다 있다면 가장 마지막(아래쪽) 것이 적용
3. 웹 브라우저 기본 스타일

#### 인라인 스타일 (Inline style)
태그 내 `style` 속성을 이용하는 방법.
...
`<link>` 요소를 통해 외부 css 파일을 적용하는 방법.
...
### Javascript
`<script>`를 사용해서 HTML 문서에 javascript를 넣을 수 있다.
...
`src` 속성을 사용해서 외부 js파일을 참조할 수 있다. src를 사용하면 태그 내 스크립트는 무시된다.
```
...
`<noscript>`는 스크립트 미지원시 표시되는 문자열을 설정한다.
```
<noscript>
...
### XHTML
`XHTML(EXtensible HTML)`은 문법이 보다 엄격한 HTML이다. 기존 HTML에서 허용하던 기법이 XHTML에선 안될 수 있다.

- XHTML DOCTYPE을 반드시 명시해야 한다.
- 모든 태그는 순서대로 닫혀 있어야 한다.
- 속성 이름은 반드시 소문자여야 한다.

이 외에 여러 가지 변경사항이 있다.

## HTML5
`HTML5`는 HTML의 최신 표준이다.
...
- 문자셋 선언 (HTML5의 기본 문자 인코딩 방식은 UFT-8)
```
<meta charset="UFT-8">
```

### 의미 (Semantic)
HTML5에선 레이아웃을 위한 별도의 요소인 의미(semantic) 요소를 제공한다. 의미 요소는 그 자체로 의미를 가지며, 브라우저나 독자에게 해당 요소의 의미를 명확히 전달할 수 있다. 의미 요소의 예시는 다음과 같다.
...
### 입력 (Input)
HTML5에서 추가된 `<form>`, `<input>`의 속성이 있다.
...
##### datalist
`<datalist>`는 미리 정의된 옵션 리스트다. 다른 input 요소에 datalist를 추가할 수 있으며, 추가 방법은 다음과 같다.
...
##### keygen
`<keygen>`은 전송될 입력 데이터를 암호화한다. 서버 측은 keygen 요소에서 생성된 key를 통해 데이터를 복호화할 수 있다.
...
#### 속성
HTML5에서 추가된 `<input>`의 type 속성값이 있다.
...
##### color
색상을 입력받는다. value 속성은 색상을 값으로 한다. (기본 값은 검정)
...
##### date
날짜를 입력받는 캘린더를 제공한다. value 속성은 날짜를 값으로 한다.
...
### 멀티미디어 (Multimedia)
#### 비디오 (Video)
HTML5에서 지원하는 비디오 파일 형식은 MP4, WebM, OGV다.
...
#### 오디오 (Audio)
HTML5에서 지원하는 오디오 파일 형식은 MP3, WAV, OGG다.
...
#### 플러그인 (Plug-in)
HTML 플러그인(plug-in)은 웹 브라우저의 표준 기능을 확장해 주는 프로그램이다. (ex. Java Applet, Flasg Player 등)

플러그인을 추가하는 방법은 2가지가 있다.
- `<object>` 사용
- `<embed>` 사용

##### object
`<object>`를 사용해서 HTML 문서에 객체를 넣을 수 있다.
...
##### embed
`<embed>`를 사용해서 HTML 문서에 객체를 넣을 수 있다.
...
#### Canvas
`<canvas>`는 웹 페이지에 그래픽을 담는 컨테이너이며, 다양한 그래픽을 표시할 수 있다.

canvas 제어는 자바스크립트로 진행한다. canvas의 2d context를 통해 평면 그래픽을 그릴 수 있다.
...
#### SVG
`SVG(Scalable Vector Graphics)`는 XML 기반 W3C 그래픽 표준 권고안으로, 벡터 이미지를 표현할 수 있다. 벡터 이미지는 픽셀의 영향을 받지 않는다. `<svg>`를 사용해서 SVG를 그릴 수 있다.
...
### API
HTML5는 자바스크립트로 구현된 API를 새롭게 제공한다.

#### geolocation API
`geolocation API`는 사용자 동의하에 사용자의 현재 위치 정보를 제공하는 API다. HTTPS에서만 사용 가능하다.
...
#### drag and drop API
드래그와 드롭 기능을 제공하는 API.

#### web storage API
웹 브라우저가 직접 데이터를 저장하는 방법을 제공하는 API로, 기존 브라우저 쿠키의 문제점을 보완한다.
...
웹 페이지가 갱신된 서버 정보를 자동으로 받는 방법을 제공하는 API.
