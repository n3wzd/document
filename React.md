...
- Effect는 무겁고 비용이 높으므로 가급적 피하는 것이 좋습니다. 렌더링할 때마다 데이터를 변경하거나, 사용자 이벤트를 처리할 때는 다른 Hooks으로 대체할 수 있습니다.
...
### Custom Hook
커스텀 훅(Custom Hook)은 React에서 로직을 재사용하기 위한 강력한 방법 중 하나입니다. 함수 컴포넌트에서 상태 관리, 효과(effect), 그리고 다른 컴포넌트 로직을 추상화하여 새로운 훅으로 만들어서 사용할 수 있게 해줍니다.
