...
## 문제
https://www.acmicpc.net/problem/23290<br>
...
void sharkMove() {
    ...
    for (int dist = 0; dist < 3; dist++) {
        ...
        if (getAllFish(sharkPos.x, sharkPos.y) > 0) {<br>
...
int main() {
    ...
    cin >> M >> S;<br>
    while (M--) {
        int fx, fy, fd; cin >> fy >> fx >> fd;<br>
        board[fy - 1][fx - 1].fish[fd - 1]++;
    }
    cin >> sharkPos.y >> sharkPos.x; sharkPos.y--, sharkPos.x--;<br>
    while (S--) {
        for (int y = 0; y < 4; y++)
            for (int x = 0; x < 4; x++)
                startCopyMagic(x, y);
